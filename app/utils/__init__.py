...
def convert(file):
    ...
    df_coaches_profile = pd.read_json('../data/backend/coaches.json')
    df_locations_profile = pd.read_json('../data/backend/locations.json')
    df_programs_schedule = pd.read_json('../data/backend/programs.json')
    print('Boom')
    ...
    grouped = df_curr_day_programs.groupby('assigned_coaches')
    ...
    curr_job_time = (df_curr_day_programs.groupby('assigned_coaches')['job_time'].sum())
    ...
    for index,row in df_curr_day_programs.iterrows():
        if not pd.isnull(row['assigned_coaches']):
            continue
        # print('Balle')
        ...
        grouped = df_curr_day_programs.groupby('assigned_coaches')
        coaches_curr_day_busy_schedule = grouped.apply(lambda row: sorted([Event('Program',start,end,lat,lon,loc_acr,court_loc) for start, end, lat,lon,loc_acr,court_loc in get_event_time_location(row)], key=lambda x: x.end_time))
        df_coaches_total_minutes = df_curr_day_programs.groupby('assigned_coaches')['Total Time'].sum()
        ...
        df_curr_day_programs.loc[index, 'assigned_coaches'] = sorted_coaches[0] if len(sorted_coaches)>0 else None
        ...
        if df_curr_day_programs.loc[index, 'assigned_coaches']:
            coach_id = df_curr_day_programs.loc[index, 'assigned_coaches']
